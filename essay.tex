...
% TODO List:
% use DDT elsewhere
% Sort out weight / filt esp section results
% optimal

% Later TOD List:
% add subsecs example, generl to sec 3, example 1 and 2 to sec 5?
% nothing unrefed
...
There is also a chapter by Ribet--Stein \cite{RibetStein} and a \cite{RibetSteinBook}. %TODO separate more expositary, move researchy ones to relevant sections.
Alex Ghitza has prepared a translation of part of Serre's paper \cite{Ghitza} which has been helpful.
All of these articles were of great help when preparing the current essay and they were of great help.
...
We use Serre and Swinnerton-Dyer's original approach here  \cite{SD,Serre73,Serre73Formes}.
...
\]
we may lift to a character %TOD teich?
\[
...
There are many good references for this type of material, for example \cite{DDT,WieseGal}. %TOD something classical?
...
We therefore obtain a map %TODO why surj?
\[
...
The groups we have been looking at give well known extensions on the Galois side.
The fixed field of $I_\ell$ is maximal non-ramified extension of $\Ql$, denoted $\Ql^\nr$.
Similarly, the fixed field of $I_{\ell,w}$ is maximal tamely-ramified extension of $\Ql$, denoted $\Ql^\tamr$.
...
\begin{defn}\label{def:higher} %TODO
Given a finite Galois extension $G = \Gal(L/\QQ)$ of number fields we may define a series of subgroups of $G$ that study the higher ramification at a prime $\ell$ of $\QQ$.
Let $P$ be the extension of the $\ell$-adic valuation to $\Qlb$ and define
\[
G_{\ell,u} = \{\sigma \in G : \nu_l(\sigma(x) - x) \ge u + 1\ \forall x \in \cO_{K} \}.
\]
\end{defn}
The $G_{\ell,u}$ form a descending chain as $u$ ranges over the integers
%that includes several groups we have already mentioned
\[
G = G_{\ell,-1} \supseteq G_{\ell,0} \supseteq G_{\ell,1} \supseteq G_{\ell, 2}\supseteq\cdots.
...
Finally, $\chi_p$ is defined to factor through $\Gal(\QQ(\zeta_p)/\QQ)$, this extension is tamely ramified and hence so it $\chi_p$.
Therefore $\chi_p$ passes to a character on the group $I_{p}/I_{p,w} = I_{p,t}$.
Straight from the definition we see that $\chi_p$ is a level 1 character of $I_{p,t}$.
\end{rmk}
...
So this product will always be the unique fundamental character of level 1, i.e. the cyclotomic character.
\end{rmk}

%TODO why can we write any char of level as fundam prod
%TOD concequences of fund chars
...
There is a unique maximal tamely ramified extension of $\Qp^{\nr}$ that is contained inside of $K$, we write $K_t$ for this extension, and we have the following setup % \ar@{-}@/_3.2pc/[ddd]_{I_p}
\[
\xymatrix{
...
\]
for all $i$ then we say that $\rho$ is \emph{peu ramif\'e}, otherwise if any $\nu_p(x_i)$ is coprime to $p$ then we say it is \emph{tr\`es ramif\'e}.
\end{defn}
...
Peter Swinnerton-Dyer writes that they actually ``do little to explain why such congruences occur'' \cite{SD}.
...
\]
for all $\ell \nmid Np$ (here viewing $\varepsilon$ as character of $\GQ$ now) and by applying \cref{thm:cheb} we see that in fact
\[
...
One interesting consequence of this type stems from the fact that Deligne's construction of mod $p$ Galois representations from modular forms is actually of $p$-adic representations.
...
%TOD elaborate on importance / use
%TOD ramakrishna?
...
However we will not cover this case here as it is not what the refined conjecture deals with.
...
If correct however, the refined form is eminently more useful than the qualitative form for any practical applications.
...
We will now spend some time giving Serre's recipe for the weight, level and character.
First however we will look at some results that will lead us to this recipe.
...
Concerning the conductor of $\rho_f$ there is the following nice result due to Carayol and Livn\'e \cite{Carayol, Livne}.
...
\\[12pt] \noindent
In all the theorems we have just introduced the weight $k$ is at most $p+1$, of course we can make Galois representation from heigher weight eigenforms.
...
To do this we recall that the $\Theta$ operator preserves the set of mod $p$ normalised cuspidal eigenforms of a particular level, but in most cases it actually increases the weight of the eigenform.
...
\end{align*}
By \cref{thm:cheb} we have that the representations involved are isomorphic.
\end{proof}
...
Then for each prime $\ell$ we let $G_{\ell,i}\subset G$ be the $i$th ramification group at $\ell$ of $G$, as defined in \cref{def:higher}. %TODO
Now consider the fixed subspace of $V$ for each $\ell$ and $i$,
\[
...
The general strategy of our approach is to express a representation of Serre-type as a twist of another representation, one that looks like it comes from a cusp form of low weight.
We then read off the minimal weight of a cusp form that could give this twisted representation.
Then we apply the results above regarding the $\Theta$ operator (\cref{prop:theta}) to define the weight of the original representation.
...
To see this, assume otherwise and consider some fixed subspace of the vector space realising $\rho_p$.
By assumption this space must be 1-dimensional, so the representation acts by a character $\phi$. %TODO
...
So $\rho_p = \rss_p$ and the characters $\phi_1$ and $\phi_2$ above define the representation $\rho_p|_{I_{p}}$.
...
If $a = b$ then $\phi_1 = (\psi_1 \psi_2)^a$, which contradicts $\phi_1$ being of level 2 as $\psi_1\psi_2$ is the level 1 cyclotomic character (recall \cref{rmk:prodchar}).
Now we observe that
\[
...
So we massage our representation into the form seen in the theorem by factoring some characters out to get
\[
\rho_p|_{I_{p}} \sim
\begin{pmatrix}
\psi_1^b\psi_2^a & 0 \\
...
\end{pmatrix}
\]
If we were just considering the rightmost matrix on we would like to set $k(\rho_p) - 1 = b - a$, however we have twisted by $\chi_p^a$.
...
This looks a similar to the ordinary case we covered in \cref{thm:ordinary}.
...
It is possible that $a = b = 0$, in which case this definition would give us $k(\rho_p) = 1$.
However we do not wish to consider weight 1 forms at all here (indeed we only attached Galois representations to forms of weight at least 2), so our formula needs modifying in this case.
...
We have one final adjustment to make, if $p=2$ this definition would give $k(\rho_p) = 3$, in Serre's definition he changes this to be $4$ instead.
...
The mod 3 Galois representation attached to $f$ has determinant $\chi_3 \phi$, where $\phi$ is the Galois character coming from the extension $\QQ(\sqrt{13})/\QQ$.
...
The order of a character for such a form must then divide 2.
...
This problem is fairly isolated and only arises when we work with mod~2 Galois representations, or mod~3 Galois representations that have abelian restriction to $\Gal(\Qb/\QQ(\sqrt{-3})$. %TODO
...
This problem is not a serious issue with the validity of the conjecture in general, if one simply ignores the character completely or is a bit careful this can be completely resolved.
...
%TOD Regarding the difficulties with the character mentioned in  \cref{subsec:counter} it is shown in ???? that if the refined version without the character condition holds then it holds with a modified character condition.
%TOD The main theorems of \cite{KWI,KWII} do not mention the character at all but combining them with the work of ???? gives the full \cref{conj:ref} excepting the cases discussed.
...
\begin{ex}
Let's return first to \cref{ex:delt} which concerned the cusp form $\Delta$, and check that everything we have just done is consistent with what we saw there. %TOD warning, it is consistent so the content of this example will be entirely tautological
...
We use that the field $K$ is given by adjoining a root of $x^6 + ?? + ??$ to $\QQ$.
This factors mod 23 as
...
\]
We can turn this into a mod 2 Galois representation using the fact that $A_4$ is isomorphic to $\GL_2(\FF_4)$ via the identification %TODO
\[
...
However this modularity statement is significantly stronger and uses the full power of Serre's conjecture. %TODO check

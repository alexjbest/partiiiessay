...
% TODO List:
% use DDT elsewhere
% maybe more careful with G_l notation
...
There are other articles coverng this version of the conjecture, such as Cais \cite{Cais} and Edixhoven \cite{Edixhoven}. %TODO more?
...
Given a finite Galois extension $K/\QQ$ we may define a series of subgroups of $G = \Gal(K/\QQ)$ that study the ramification of $K/\QQ$ at a prime $\ell$. %TODO what is ramification?!?!? chicken egg
...
%TODO These groups generalise the corresponding notions for finite Galois extensions, in the sense that we have the following equalities.
%\[
%\Gl/
%\]
...
Similarly, the fixed field of $I_{\ell,w}$ is the maximal tamely-ramified extension of $\Ql$, denoted $\Ql^\tamr$. %TODO
...
\[
\lim_{\longleftarrow} \Gal(\Ql^\nr(\sqrt[n]{\ell})/\Ql^\nr)= \lim_{\longleftarrow} \FF_{\ell^n}^*. %TODO
\]
%TODO maps are norms 
...
%TOD do we want also to note that all chars factor through I_t
...
These fields fit together as follows % \ar@{-}@/_3.2pc/[ddd]_{I_p}
\[
\xymatrix{
...
\]%TODO should we rly be working with p-adic cyclo
...
This provided an indication that using Galois representations is a natural and often effective way of thinking about questions regarding $\tau$. %TODO
...
\]
By looking at the action of $\langle-1\rangle$ on $f$ we find $\varepsilon(c)f = \langle -1 \rangle f = (-1)^k f$, and so%TODO
\[
...
This is already a very useful thing to know, any statement one could prove about Galois representations attached to  modular forms, by using the theory of these forms for example, would hold for all odd 2-dimensional mod $p$ Galois representations. %TOD see anywhere?
...
It was computations performed by J.-F. Mestre that convinced Serre that strengthening this conjecture was plausible, and this led to the form we are about to see.%TODO
...
This subspace is stable under the action of $\Gp$, as is the space $V/V^{I_{p,w}}$, so we may write %TODO why?
\[
...
%TODO In order to determine $k(\rho)$ we need to study the local representation at 23.
%We use that the field $K$ is given by adjoining to $\QQ$ a root of
%\[
%x^{6} - 3 x^{5} + 5 x^{4} - 5 x^{3} + 5 x^{2} - 3 x + 1.
%\]
%This factors mod 23 as $(x + 1)^2(x + 11)^2(x + 21)^2$ and hence the decomposition group is of order .
%The inertia group is also
\[
\rho_{23}|_{I_{23}} \sim
\begin{pmatrix}
\phi & 0 \\
...
%TODO Why irred?
%TODO First the determinant of $\rho$, .
...
%While not all of these results need the fuwere obtained via other means long before Serre's conjecture was shown in general they still serve to demonstrate a little the strength and utility of the conjecture.
